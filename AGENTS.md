...
제품 방향, 개선 제안, A/B 실험 가설, 비교 결과처럼 이후 의사결정에 영향을 주는 내용은 대화에만 남기지 않는다. 관련 내용을 `docs/`의 명확한 단일 문서로 정리하고, 생성/갱신 사실을 `docs/state.md`와 `docs/task-board.md`에 기록해 히스토리를 남긴다.

운영 문서와 제품 방향 문서는 기본적으로 한글로 작성한다. 외부 API 이름, 코드 식별자, route, 버튼 label처럼 원문 유지가 필요한 항목만 영어를 그대로 둔다.
